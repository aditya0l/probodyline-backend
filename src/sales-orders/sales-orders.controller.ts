import { Controller, Post, Patch, Param, Body, BadRequestException, Get, Delete } from '@nestjs/common';
...
export class SalesOrdersController {
    ...
    createSplit(@Body() body: { quotationId: string; itemIds: string[]; dispatchDate?: string }) {
        return this.salesOrdersService.createSplit(body.quotationId, body.itemIds, body.dispatchDate);
    }

    @Patch(':id/update-split')
    @ApiOperation({ summary: 'Update items or date for a Sales Order (if not booked)' })
    updateSplit(
        @Param('id') id: string,
        @Body() body: { itemIds: string[]; dispatchDate?: string }
    ) {
        return this.salesOrdersService.updateSplit(id, body.itemIds, body.dispatchDate);
    }
